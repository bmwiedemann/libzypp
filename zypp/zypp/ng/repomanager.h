...
namespace zyppng {
   ...
   *
   * Depending on the \a ContextRef the convenienve functions execute the workflows in
  ...
  public:
    ...
    inline static expected<std::shared_ptr<RepoManager>> create ( Args && ...args ) {
      using namespace zyppng::operators;
      auto mgr =  std::make_shared< RepoManager >( private_constr_t{}, std::forward<Args>(args)... );
