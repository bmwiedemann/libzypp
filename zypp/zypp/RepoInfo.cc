...
{ /////////////////////////////////////////////////////////////////
  ...
  public:
    ...
    RepoVariablesReplacedUrlList & mirrorUrls() const
    {
      ...
      _mirrorUrls.clear();
      _lastMirrorUrlsUpdate = std::chrono::steady_clock::now();
  ...
  public:
     ...
     * must not be dumped to an INI file or XML.
     */
    ...
    void forDumpableExtraValues( Fnc && fnc_r ) const
    {
      for ( const auto & [k,v] : _extraValues ) {
        ...
        if constexpr ( std::is_invocable_v<Fnc, std::string_view, std::string_view, bool> ) {
          ...
          static_assert( zyppng::always_false_v<Fnc>, "Lambda must accept (k, v) or (k, v, b)" );
        }
      }
    }
  ...
  MirroredOriginSet RepoInfo::repoOrigins() const
  {
    return _pimpl->repoOrigins();
  }

  bool RepoInfo::repoOriginsEmpty() const
  {
    return ( _pimpl->baseUrls().empty () && _pimpl->mirrorUrls().empty() );
  }
...
#undef OUTS
  ...
  std::ostream & RepoInfo::dumpAsXmlOn( std::ostream & str, const std::string & content ) const
  {
    ...
    _pimpl->forDumpableExtraValues( [&]( std::string_view k, std::string_view v ) {
      str << "<extra key=\"" << escape(std::string(k)) << "\">"
          << escape(std::string(v))
          << "</extra>" << endl;
    } );
...
#undef OUTS
  ...
  { return _pimpl->setExtraValue( key_r, std::move(value_r) ); }
